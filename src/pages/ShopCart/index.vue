...
export default {
  ...
  methods: {
    ...
    handler: throttle(async function (type, disNum, cart) {
      // type:区分是加还是减还是改变数字
      // disNum形参：加为变化量+1，减为变化量-1，input是最终的个数（并不是变化量）
      // cart：确定改变的是哪一个产品
      // 向服务器发请求修改数量
      switch (type) {
        // 加号
        ...
        case 'minus':
          ...
          // 若产品个数小于等于1，则传给服务器0（原封不动）
          disNum = cart.skuNum > 1 ? -1 : 0
          break
        case 'change':
          // 用户输入进来的最终量（非法的，带有汉字/出现负数）
  ...
  computed: {
    ...
    // 计算总价
    totalPrice() {
      return this.cartInfoList.reduce((pre, cur) => pre + cur.skuNum * cur.skuPrice, 0)
    },
    // 判断底部全选框是否勾选
    isAllChecked() {
